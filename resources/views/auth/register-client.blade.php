                    ...
                    <div class="card card-bordered mt-4">
                        <div class="card-inner card-inner-lg pt-3 pb-4">
                            <div class="nk-block-head pb-0">
                                <a href="{{route('login.index', ['type' => 'client'])}}" class="tw-flex tw-items-center tw-text-slate-700">
                                    <div class="tw-slate-700"><em class="icon ni ni-arrow-left tw-text-lg me-2"></em></div>
                                    <div>Back to Login</div>
                                </a>
                            </div>
                            <div class="nk-block-head py-2">
                                <div class="center fw-bold fs-18px tw-text-slate-700 tw-underline">Client Registration</div>
                            </div>
                            <div class="row gx-2">
                                <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-control-wrap">
                                    <div class="input-group">
                                        ...
                                        <input id="phone" class="form-control" placeholder="Eg: Phone Number" value=""/>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        $('#signInSelect').on('change', function () {
            if ($(this).val() === 'user') {
                ...
                $('.btn-primary').html('Client Login');
                $('.btn-primary').attr('onclick', 'loginClient()');
            }
        })
        ...
        function register() {
            ...
            let refererCode = '{{request()->query('ref')}}';
            $.ajax({
                url: "{{ route('register.store','client')}}",
                method: 'POST',
                data: {
                    name: name,
                    email: email,
                ...
                success: function (response) {
                    _Swal.success('Register Success', 'You have registered successfully, you can now login as Client. Don\'t forget to confirm your email.', () => {
                        window.location.href = "{{route('login.index', ['type' => 'client'])}}";
                    });
                },
