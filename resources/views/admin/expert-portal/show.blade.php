                            ...
                            <div class="col-3">
                                <div class="user-card user-card-s2 pt-0 pb-0">
                                    <div class="user-avatar lg bg-light !tw-rounded-lg !tw-w-32 !tw-h-32 ">
                                        <img class=" !tw-w-32 !tw-h-32 !tw-rounded-lg !tw-bg-slate-200" src="{{str_contains($expert_user->img_url, 'https') ? $expert_user->img_url : $expert_user->img_url ?? '/images/svg/avatar.svg' }}" alt="" id="avatar" referrerpolicy="no-referrer">
                                    </div>
                                    <div class="mt-2">
                                        ...
                                        <h5 id="name" class="mb-0">{{$expert_user->name ?? 'N/A'}}</h5>
                                        <div class="mb-1" id="email"><i class="fa-solid fa-envelope me-1"></i>
                                            {{$expert_user->email ?? 'No email contact'}}
                                            <span class="data-more clickable" onclick="updateEmail()"><i class="ms-2 fa-solid fa-pen-to-square text-info !hover:tw-bg-slate-500"></i></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="row text-center">
                                        <div class="col-4">
                                            <div class="card card-bordered bg-info py-1">
                                                <span class="fw-bolder text-white fs-20px" id="project_total">{{$expert_user->total_project ?? 0}}</span>
                                                <span class="sub-text text-white">Total Project</span>
                                            </div>
                                        </div>
                                        <div class="col-4">
                                            <div class="card card-bordered bg-success py-1">
                                                <span class="fw-bolder text-white fs-20px" id="project_completed">{{$expert_user->total_completed ?? 0}}</span>
                                                <span class="sub-text text-white">Completed Project</span>
                                            </div>
                                        </div>
                                        <div class="col-4">
                                            <div class="card card-bordered bg-warning py-1">
                                                <span class="fw-bolder text-white fs-20px" id="project_ongoing">{{$expert_user->total_ongoing ?? 0}}</span>
                                ...
                                <div class="card-inner">
                                    ...
                                    <div class="row g-3">
                                        <div class="col-sm-3">
                                            ...
                                            <a href="{{$expert_user->url}}" id="linkedin" class="tw-text-blue-500" target="_blank"><i class="text-info me-1 fa-brands fa-linkedin fs-16px"></i><span class="fs-14px">{{str_replace('https://www.linkedin.com/in/', '', $expert_user->url)}}</span></a>
                                        </div>
                                        <div class="col-sm-3">
                                            <span class="sub-text">Country:</span>
                                            <span class="fs-16px" id="country">{{$expert_user->country}}</span>
                                        </div>
                                        <div class="col-sm-3">
                                            <span class="sub-text">Address:</span>
                                            <span class="fs-14px" id="last_login">{{$expert_user->address}}</span>
                                        </div>
                                        <div class="col-sm-3">
                                            <span class="sub-text">Register At:</span>
                                            <span class="fs-14px" id="register_at">{{ $expert_user->registered_at ?? 'Not registered' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="card card-bordered">
                    <div class="card-inner">
                        <ul class="nav nav-tabs mt-n3">
                            <li class="nav-item">
                                <a class="nav-link active" data-bs-toggle="tab" href="#tabDetails">Expert Details</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-bs-toggle="tab" href="#tabHistoryOngoing">Ongoing Project</a>
                            </li>
                            <li class="nav-item">
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane row active" id="tabDetails">
                                <div class="col-12">
                                    <h6 class="fs-16px text-info"><i class="fa-solid fa-user-tie me-1"></i>About</h6>
                                    <p class="fs-13px" id="expert_about">{{$expert_user->about ?? '-'}}</p>
                                </div>
                                <div class="col-12 mt-4">
                                    <h6 class="fs-16px text-info"><i class="fa-solid fa-industry me-1"></i>Industry</h6>
                                    <div class="d-flex gx-2">
                                        <div>Main Industry: <span class="badge badge-sm rounded-pill bg-outline-secondary me-1 mb-1 !tw-py-0.5" id="expert_main_industry">{{$expert_user->industry_main ?? 'Not set'}}</span></div>
                                        <div>Sub Industry: <span class="badge badge-sm rounded-pill bg-outline-secondary me-1 mb-1 !tw-py-0.5" id="expert_sub_industry">{{$expert_user->industry_sub ?? 'Not set'}}</span></div>
                                    </div>
                                </div>
                                <div class="col-12 mt-4">
                                    ...
                                    <div class="nk-tb-list nk-tb-ulist is-compact border round-sm" id="expert_experiences_container">
                                        ...
                                        @foreach($expert_user->experiences as $experience)
                                ...
                                <div class="col-12 mt-4">
                                    ...
                                    <div id="expert_skills">
                                        @foreach($expert_user->skills as $skill)
                                            <span class="badge badge-sm rounded-pill bg-outline-secondary me-1 mb-1 !tw-py-0.5">{{$skill}}</span>
                                        @endforeach
                                    </div>
                                </div>
                                <div class="col-12 mt-4">
                                    <h6 class="fs-16px text-info"><i class="fa-solid fa-briefcase me-1"></i>Languages</h6>
                                    <div id="expert_languages">
                                        @foreach($expert_user->languages as $language)
                                            <span class="badge badge-sm rounded-pill bg-outline-secondary me-1 mb-1 !tw-py-0.5">{{$language}}</span>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        function updateEmail() {
            Swal.fire({
                ...
                preConfirm: (email) => {
                    $.ajax({
                        url: `{{route('admin.expert-portal.update', ':id')}}`.replace(':id', '{{$expert_user->id}}'),
                        type: 'PUT',
                        data: {
                            email: email,
                            _token: '{{ csrf_token() }}'
