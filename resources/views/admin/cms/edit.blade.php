...
@section('content')
    ...
    <x-content_header title="Edit Post" back="{{route('post.show')}}"
                      subtitle="Edit your Article/ Blog/ Use Case to show to Asia Deal Hub Website"/>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <ul class="align-center flex-wrap flex-sm-nowrap gx-4 mt-1">
                                        <li>
                                            <button id="submitBtn" class="btn btn-primary">Edit Post</button>
                                        </li>
                                    </ul>
                                </div>
                            </div>
            ...
            <div class="col-lg-8">
                <div class="card card-bordered tw-h-full">
                    <div class="card-inner">
                        <x-tinymce-editor content="{!! $page->content !!}"/>
                    </div>
                </div><!-- .card -->
            </div><!-- .col -->
...
@push('scripts')
    <script>
        ...
        $('#addTag').val([{{$page->tags}}]).trigger('change');
        $('#pageAuthor').val('{{$page->author_id}}').trigger('change');
        ...
        $('#submitBtn').click(e => {
            ...
            $.ajax({
                ...
                success: response => {
                    if (response.success) {
                        Swal.fire('Update Post Success', response.message, 'success');
                        setTimeout(() => window.location.href = '{{route('post.create')}}', 1000);
                    }
                },
                error: response => {
