        ...
        <div class="nk-block-between">
            <div class="nk-block-head-content">
                ...
                <div class="nk-block-des text-soft">
                    <p>Project Title: <span class="text-base">{{ $project->name }}</span></p>
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            getCompanyDetail({{ $project->company_id }});
            $('#company_holder').removeClass('d-none');
            $('#project-name').val('{{ $project->name }}');
            $('#project-description').val('{{ $project->description }}');
            $('#hub').val('{{ $project->hub_id }}').trigger('change');
            $('#deadline').datepicker('setDate', new Date('{{ $project->deadline }}'));
            $('#communication_language').val('{{ $project->communication_language }}'.split(',')).trigger('change');
            $('#target_country').val([{{ collect($project->targetCountries->pluck('id')->toArray())->implode(',') }}]).trigger('change');
            $('#target_company_size').val('{{ $project->projectTargetInfo->company_size }}').trigger('change');
        });
        ...
        function initIndustry() {
            ...
            $('#target_industry').select2({
                data: industries.map(function(item) {
                    return {
                        id: item.id,
                        text: item.name,
                    }
        ...
        $(document).ready(function() {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    console.log('{{ $project->projectTargetInfo->industry->id }}')
                    {{--$('#main-industry').val('{{ $project->projectTargetInfo->industry->id }}').trigger('change');--}}
                }
            });
        });
