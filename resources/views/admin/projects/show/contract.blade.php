    ...
    <div class="card-inner row">
        <div class="col-6">
            <div>
                ...
                @if (
                    $project->contract &&
                        $project->contract->where('type', 'client')->whereIn('status', ['active', 'signed'])->first())
                        ...
                        <div class="alert alert-success alert-dismissible" role="alert">
                            <div class="alert-message">
                                <p class="mb-1"><em class="ni ni-info me-1"></em>Signed contract has been uploaded by
                                    client. Click below to view the contract.</p>
                                <a href="{{ config('app.url') }}/contracts/{{ $project->contract->where('type', 'client')->where('status', 'signed')->first()->filepath }}"
                        ...
                        <div class="alert alert-warning alert-dismissible" role="alert">
                            <div class="alert-message">
                                <p><em class="ni ni-info me-1"></em>Contract successfully upload submitted to the
                                    Client. Please wait for client to upload the signed contract for this project.</p>
                            </div>
                        </div>
                    @endif
                @else
                    <div class="alert alert-warning alert-dismissible mb-1" role="alert">
                        <div class="alert-message">
                            <p><em class="ni ni-info me-1"></em>Project contract for client not created yet. <a
                                    class="tw-text-blue-500 tw-cursor-pointer" onclick="createContract('client')">Edit
                    ...
                    <div class="form-control py-2">
                        ...
                        <div class="form-control-wrap">
                            <div class="input-group">
                                <input type="text" class="form-control" id="client_amount"
                ...
                success: response => {
                    Swal.fire('Contract Saved', response.message, 'success').then(() => {
                        window.location.href =
                            '{{ route('admin.projects.show', ['pid' => $project->pid]) }}';
                    });
                },
                error: response => {
                    Swal.fire('Error', response.responseJSON.message, 'error')
