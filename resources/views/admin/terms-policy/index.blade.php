    ...
    <div class="nk-block-head nk-block-head-sm">
        <div class="nk-block-between">
            <div class="nk-block-head-content">
                <h3 class="nk-block-title page-title">{{$types}} Editor</h3>
                <div class="nk-block-des text-soft">
                    <p>Edit or change your privacy policy and terms conditions.</p>
                </div>
            </div>
            <div class="nk-block-head-content">
                <div class="toggle-wrap nk-block-tools-toggle">
                    <div class="toggle-expand-content" data-content="pageMenu">
                        <ul class="nk-block-tools g-3">
                            <li class="nk-block-tools-opt"><a href="{{route('admin.terms-policy.create', key(request()->query()))}}" class="btn btn-primary"><em class="icon ni ni-plus"></em><span>Create {{$types}}</span></a></li>
                        </ul>
                    </div>
                </div>
            </div>
...
@push('scripts')
    ...
    <script>
        datatableInit('#datatable', {
            ajax: '{{route('admin.terms-policy.index', key(request()->query()))}}',
            simpleTable: true,
            columnDefs: [
                {"className": "nk-tb-col py-2", "targets": "_all"},
                {
                    ...
                    "createdCell": function(td, cellData, rowData) {
                        $(td).on('click', () => window.location.href =
                            '{{ route('admin.terms-policy.edit', ':id') }}?{{key(request()->query())}}'.replace(':id', rowData.id));
                    }
                },
            ],
            columns: [
                                    ...
                                    <div class="drodown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end">
                                            <ul class="link-list-opt no-bdr">
                                                <li><a href="{{route('admin.contract-template.show', '')}}/${data}"><em class="icon ni ni-eye"></em><span>View</span></a></li>
                                                <li><a onclick="deleteContent('${data}')"><em class="icon ni ni-trash"></em><span>Delete</span></a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </li>
        ...
        function addTemplate() {
            window.location.href = '{{route('admin.contract-template.create')}}';
        }

        function deleteContent(id) {
            Swal.fire({
                title: 'Delete this Policy & Terms?',
                text: 'Are you sure you want to delete this contract template? Make sure you know what are you doing',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{route('admin.terms-policy.destroy', ':id')}}/'.replace(':id', id),
                        method: 'DELETE',
                        headers: {
                        ...
                        success: function (response) {
                            Swal.fire({
                                title: 'Success',
                                text: 'Terms policy deleted successfully',
                                icon: 'success',
                        ...
                        error: function (response) {
                            Swal.fire({
                                title: 'Error',
                                text: 'Failed to delete contract template',
                                icon: 'error',
                                showConfirmButton: false,
                                timer: 1500
                            });
