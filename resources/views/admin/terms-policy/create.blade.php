    ...
    <div class="nk-block-head nk-block-head-sm">
        <div class="nk-block-between">
            <div class="nk-block-head-content">
                <h3 class="nk-block-title page-title tw-capitalize">Create {{$types}}</h3>
                <div class="nk-block-des text-soft">
                    <p>Create new {{$types}} for another languages</p>
                </div>
            </div>
            <div class="nk-block-head-content">
                <div class="toggle-wrap nk-block-tools-toggle">
                    <div class="toggle-expand-content" data-content="pageMenu">
                        <ul class="nk-block-tools g-3">
                            <li class="nk-block-tools-opt"><a onclick="saveContract()" class="btn btn-primary"><em class="icon ni ni-save"></em><span>Save {{$types}}</span></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        function saveContract(){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{route('admin.terms-policy.store')}}',
                        method: 'POST',
                        data: {
                            content: content,
                        ...
                        success: response => {
                            if (response.success) {
                                Swal.fire('Upload Success', response.message, 'success').then(() => {
                                    window.location.href = '{{route('admin.terms-policy.index', key(request()->query()))}}';
                                });
                            }
                        },
                        error: response => {
