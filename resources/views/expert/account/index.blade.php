            ...
            <div class="nk-data data-list">
                ...
                <div class="data-item">
                    <div class="data-col">
                        ...
                        <span class="data-value {{auth()->user()->phone ? '':'text-soft'}}">{{auth()->user()->phone ?  (auth()->user()->phone_code . auth()->user()->phone) : 'Not set yet'}}</span>
                    </div>
                    <div class="data-col data-col-end"><span class="data-more" onclick="changePhone()"><em class="icon ni ni-pen"></em></span></div>
                </div>
            </div>
        </div>
        <div class="nk-block-head">
    ...
    <script>
                    ...
                    reader.onload = (e) => {
                        ...
                        img.onload = () => {
                            ...
                            Swal.fire({
                                ...
                                preConfirm: (image) => {
                                    $.ajax({
                                        url: '{{route('expert.account.avatar')}}',
                                        type: 'POST',
                                        data: {
                                            _token: '{{csrf_token()}}',
        ...
        function changeLanguage() {
            Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    $.ajax({
                        url: '{{route('expert.account.language')}}',
                        type: 'POST',
                        data: {
                            _token: '{{csrf_token()}}',
                            language: language
        ...
        function changeTimezone() {
            Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    $.ajax({
                        url: '{{route('expert.account.timezone')}}',
                        type: 'POST',
                        data: {
                            _token: '{{csrf_token()}}',
                            timezone: timezone
        ...
        function changePhone() {
            Swal.fire({
                ...
                html: `<div class="form-group center mt-3">
                    <input type="text" class="form-control tw-w-20 me-2" id="phone_code" name="phone_code" placeholder="Country Code" value="{{auth()->user()->phone_code}}">
                ...
                preConfirm: () => {
                    ...
                    $.ajax({
                        url: '{{route('expert.account.phone')}}',
                        type: 'POST',
                        data: {
                            _token: '{{csrf_token()}}',
                        ...
                        success: function (response) {
                            if (response.success) {
                                Swal.fire('Phone number updated!', '', 'success').then(() => {
                                    window.location.reload();
                                });
                            } else {
                                Swal.fire('Error!', response.message, 'error');
