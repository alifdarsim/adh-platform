    ...
    @if($project->contract && $project->contract->where('type', 'expert')->where('status', 'active')->first())
        <div class="alert alert-light alert-dismissible" role="alert">
            <div class="alert-message">
                ...
                <a href="{{config('app.url')}}/contracts/{{$project->contract->where('type', 'expert')->first()->filepath}}" target="_blank" class="btn btn-primary mt-0">Download Contract</a>
            </div>
        </div>
        ...
        @if($project->contract && $project->contract->where('type', 'expert')->where('status', 'signed')->first())
            <div class="alert alert-light alert-dismissible" role="alert">
                <div class="alert-message">
                    <p><em class="ni ni-check me-1"></em>Your signed contract has been uploaded</p>
                </div>
            </div>
        @else
