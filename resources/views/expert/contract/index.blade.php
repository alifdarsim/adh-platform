        ...
        <div class="nk-block-between">
            <div class="nk-block-head-content">
                ...
                <div class="nk-block-des text-soft">
                    <p>See all contract information for all projects</p>
                </div>
            </div>
            <div class="nk-block-head-content">
...
@push('scripts')
    ...
    <script>
        datatableInit('#datatable', {
            ajax: '{{ route('expert.contract.index') }}',
            columnDefs: [
                {
                    "orderable": false,
                    "targets": [0, 1, 2, 3]
                ...
                },
                {
                    ...
                    "createdCell": function(td, cellData, rowData) {
                        $(td).on('click', () => window.location.href =
                            '{{ route('expert.contract.show', '') }}/' + rowData.contract_id)
                    }
                },
            ],
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function(data) {
                        return `<div href="{{ route('expert.contract.show', '') }}/${data.pid}" class="text-capitalize">${data.project.name}</a>`; // Link to project
                    }
                },
                {
                    data: 'project_expert',
                    render: function(data) {
                        return `<div class="text-capitalize">${data.expert.name}</a>`;
                    }
                },
                {
                    data: 'status',
                    render: function(data) {
                        console.log(data)
                        let color = data === 'pending' ? 'warning' : data === 'approved' ? 'info' : 'success';
                        return `<span class="text-capitalize badge bg-${color}">${data === 'approved' ? 'Waiting Signature' : 'Contract Submit'}</span>`;
                    }
                },
                {
                    data: 'contract_id',
        ...
        function view(contract_id){
            window.location.href = '{{ route('expert.contract.show', '') }}/' + contract_id;
        }
    </script>
@endpush
