function submitLoading(){
    ...
    $('button[type="submit"]').html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span><span>Loading...</span>');
}
...
const _Swal = {
    loading: function (title, html) {
        Swal.fire({
            title: title,
            html: `${html}<br><div class="spinner-grow mt-3" role="status"></div>`,
            allowOutsideClick: false,
            showConfirmButton: false,
        });
    },
    loadingCallback: function (title, html, timer, callback, icon = null) {
        Swal.fire({
            title: title,
            html: `${html}<br><div class="spinner-grow mt-3" role="status"></div>`,
            allowOutsideClick: false,
    ...
    success: function (title, html = 'Success', callback = null) {
        Swal.fire({
            title: title,
            html: html,
            icon: 'success',
    ...
    error: function (response, title = 'Error', callback = null) {
        let html = '';
        if (response.status === 422) {
            html = '<ul class="mt-1">';
            let num = 1;
            $.each(response.responseJSON.errors, function (key, value) {
                html += `<li>${num}. ${value}</li>`;
                num++;
            });
            html += '</ul>';
            html = 'Kindly make sure all the field is filled up' + html;
            title = title === null ? 'Required more Info' : title;
        } else {
            if (response.responseJSON === undefined) html = response;
            else html = response.responseJSON.message;
        }
        Swal.fire({
            title: title,
            html: html,
